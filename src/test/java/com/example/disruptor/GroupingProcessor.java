...
import static com.example.disruptor.DisruptorApplicationTests.EVENTS_POOLING;
...
public final class GroupingProcessor implements EventProcessor {
  ...
  public GroupingProcessor(
    ...
    this.waitSpinningHelper =
        new WaitSpinningHelper(ringBuffer, sequenceBarrier, GROUP_SPIN_LIMIT, coreWaitStrategy);
    this.msgsInGroupLimit = 256;
    this.maxGroupDurationNs = 10_000;
      ...
      try {
        ...
        if (nextSequence <= availableSequence) {
          while (nextSequence <= availableSequence) {
            ...
            final MyEvent cmd = ringBuffer.get(nextSequence);
            log.debug("grouping, {}...", cmd.command);

            nextSequence++;

            if (cmd.command == OrderCommandType.GROUPING_CONTROL) {
              groupingEnabled = cmd.orderId == 1;
              cmd.resultCode = "SUCCESS";
            }
            ...
            // some commands should trigger R2 stage to avoid unprocessed events that could affect
            // accounting state
  ...
  public String toString() {
    return "GroupingProcessor{" + "GL=" + msgsInGroupLimit + '}';
  }
}
